...
export const acceptableTokenList = (chainId?: number, devDeployed?: any) => {
  if (!chainId) return [];
  const dao =
    (process.env.NODE_ENV === "development" ? devDeployed : deployed) ||
    deployed;
  return [
    {
...
export const getStablecoinList = (chainId?: number, devDeployed?: any) => {
  if (!chainId) return [];
  if (process.env.NODE_ENV === "development") {
    const dao = devDeployed;
    return [
      {
        symbol: "Mock",
        address: dao[getNetworkName(chainId)].BaseCurrency as string,
