...
export const CreateLock: React.FC<CreateLockProps> = ({ stakedAmount }) => {
  ...
  const approve = () => {
    if (!!workhard && !!library && !!account) {
      ...
      handleTransaction(
        workhard.dao.vision
          .connect(signer)
          .approve(workhard.dao.votingEscrow.address, constants.MaxUint256),
        setTxStatus,
        addToast,
  ...
  useEffect(() => {
    if (!!account && !!workhard) {
      const { vision, votingEscrow } = workhard.dao;
      vision
        .balanceOf(account)
  ...
  }, [account, workhard, txStatus, blockNumber, addToast]);
        ...
        <Form>
          <Form.Group>
            ...
            <Form.Text>
              ...
              of your {workhard?.metadata.visionSymbol || "$VISION"} token is
              staked.
            </Form.Text>
          </Form.Group>
          <Form.Group>
