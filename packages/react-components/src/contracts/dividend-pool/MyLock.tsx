...
export const MyLock: React.FC<MyLockProps> = ({ index, lockId }) => {
  ...
  useEffect(() => {
    if (!!account && !!workhard) {
      workhard.dao.votingEscrow.delegateeOf(lockId).then(setDelegatee);
      workhard.dao.votingEscrow.locks(lockId).then((lock) => {
        setLockedAmount(lock.amount);
        setLockedUntil(lock.end);
        setLockedFrom(lock.start);
      });
    }
  }, [workhard, account, txStatus]);
  ...
  useEffect(() => {
    if (!!account && !!workhard) {
      const { vision, dividendPool, right } = workhard.dao;
      vision
        .balanceOf(account)
  ...
  }, [account, workhard, txStatus, blockNumber]);
  ...
  const approve = () => {
    if (!!workhard && !!library && !!account) {
      ...
      handleTransaction(
        workhard.dao.vision
          .connect(signer)
          .approve(workhard.dao.dividendPool.address, constants.MaxUint256),
        setTxStatus,
        addToast,
  ...
  const delegate = () => {
    if (!!workhard && !!library && !!account && !!delegateTo) {
      const signer = library.getSigner(account);
      handleTransaction(
        workhard.dao.votingEscrow
          .connect(signer)
  ...
  const withdraw = () => {
    if (!!workhard && !!library && !!account) {
      const signer = library.getSigner(account);
      handleTransaction(
        workhard.dao.votingEscrow.connect(signer).withdraw(lockId),
        setTxStatus,
        addToast,
