...
export const TimelockTx: React.FC<TimelockTxProps> = ({
  ...
  useEffect(() => {
    if (!!workhard && !!library) {
      try {
        const timelock = workhard.dao.timelock;
        const workersUnion = workhard.dao.workersUnion;
        library
        ...
        if (tx.to) {
          ...
          } else if (tx.to === workhard.dao.multisig.address) {
            proposer = TxProposer.DEV;
          } else {
          ...
          let result: Result;
          if (tx.to === workhard.dao.multisig.address) {
            const txDetails = workhard.decodeTxDetails(
              workhard,
              tx.to,
              tx.data,
              tx.value
            );
            const decodedGnosisSafeTx = workhard.decodeTxDetails(
              workhard,
              txDetails.result.to,
  ...
  }, [library, workhard, lastTx]);

  useEffect(() => {
    if (!!workhard && !!scheduledTx) {
      try {
        ...
        if (
          ...
          setDecodedTxData(
            target.map((_target: string, i: number) => {
              return workhard.decodeTxDetails(
                workhard,
                _target,
                data[i],
                value[i]
          ...
          setDecodedTxData([
            workhard.decodeTxDetails(workhard, target, data, value),
          ]);
        } else {
  ...
  }, [workhard, scheduledTx]);

  useEffect(() => {
    if (!!workhard && !!account && !!chainId) {
      const safe = workhard.dao.multisig.address;
  ...
  const execute = async () => {
    if (!account || !workhard || !library || !scheduledTx || !chainId) {
      alert("Not connected");
      return;
    }

    const signer: Signer = library.getSigner(account);
    const timeLockGovernance = workhard.dao.timelock;
  ...
  const cancel = async () => {
    ...
    const { target, value, data, predecessor, salt } = scheduledTx;
    const tx = await workhard.dao.timelock.populateTransaction.cancel(id);
    safeTxHandler(
      chainId,
      workhard.dao.multisig.address,
      tx,
  ...
  }, [account, workhard, scheduledTx, lastTx]);
