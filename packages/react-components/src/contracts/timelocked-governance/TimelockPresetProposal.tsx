...
import { preset, errorHandler, safeTxHandler, TxStatus } from "@workhard/react-utils";
import { getNetworkName } from "@workhard/protocol";

export const TimelockPresetProposal: React.FC<preset.Preset> = ({
  paramArray,
  methodName,
  contractName,
  contract,
  ...
  const handleSubmit: FormEventHandler = async (event) => {
    ...
    if (!account || !workhard || !contract || !chainId) {
      alert("Not connected");
      return;
    }
    const getType = (valueName: string) => {
      const argInfo: preset.Param = paramArray.filter(
        (x) => x.name == valueName
      )[0];
      return argInfo.type;
    };
    const params = Object.entries(args).map((x) =>
      preset.convertType(getType(x[0]), x[1])
    );
    const { data } = handler
