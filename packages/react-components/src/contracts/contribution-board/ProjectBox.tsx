...
export const ProjectBox: React.FC<ProjectProps> = ({ projId, active }) => {
  ...
  useEffect(() => {
    if (!!account && !!library && !!chainId && !!workhard && !!ipfs) {
      const { contributionBoard } = workhard.dao;
      workhard.project
        .tokenURI(projId)
        .then(async (uri) => {
      ...
      contributionBoard
        ...
        .catch(errorHandler(addToast));
      workhard.project.ownerOf(projId).then((owner) => {
  ...
  }, [account, library, chainId, workhard]); // ensures refresh if referential identity of library doesn't change across chainIds
