...
export const ProjectAdmin: React.FC<ProjectAdminProps> = ({
  ...
  const transferOwnership = async () => {
    if (!workhard || !account || !library || owner === "") {
      alert("Not connected.");
      return;
    }
    if (!transferTo) {
  ...
  const startInitialContributorProgram = async () => {
    if (!workhard || !account || !library || !chainId) {
      alert("Not connected.");
      return;
    }
    const signer = library.getSigner(account);
    const popTx = await workhard.dao.contributionBoard.populateTransaction.startInitialContributorShareProgram(
      projId,
      minimumShare,
