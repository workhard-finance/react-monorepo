...
export const SetEmission = () => {
  ...
  const getPresetTokenName = (address: string): string | undefined => {
    if (compareAddress(address, workhard?.periphery.visionLP.address)) {
      return "Vision LP";
    } else {
      return undefined;
    }
      ...
      workhard.dao.visionEmitter
        ...
        .then(async (num) => {
          const [_weights, _pools] = await Promise.all([
            Promise.all(
              Array(num.toNumber())
                .fill(workhard.dao.visionEmitter)
          ...
          const [_types, _baseTokens] = await Promise.all([
            Promise.all(
              _pools.map((addr) =>
                workhard.dao.visionEmitter.poolTypes(addr)
              )
            ),
            Promise.all(
              _pools.map((addr) =>
  ...
  const multisigSetEmission = async () => {
    ...
    const emissionSetTx = await workhard.dao.visionEmitter.populateTransaction.setEmission(
      {
        pools:
          pools?.map((pool) => ({
    ...
    const tx = await workhard.dao.timelock.populateTransaction.schedule(
      to,
      0,
      data,
      constants.HashZero,
