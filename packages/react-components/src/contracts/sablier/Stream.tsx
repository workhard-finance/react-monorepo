...
export const Stream: React.FC<StreamProps> = ({
  ...
  useEffect(() => {
    if (!!account && !!library && !!streamId && !!workhard && !cancelled) {
      const sablier = workhard.commons.sablier;
      sablier
        .getStream(streamId)
        .then(setStream)
  ...
  const withdraw = async () => {
    ...
    handleTransaction(
      ...
      setTxStatus,
      addToast,
      `Successfully withdrew ${workhard.metadata.commitSymbol}!`
    );
  };

  const cancel = async () => {
    if (!account || !workhard || !library || !chainId) {
      alert("Not connected");
      return;
    }
