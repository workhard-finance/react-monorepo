...
import { coingecko, errorHandler } from "@workhard/react-utils";
import { formatEther, getAddress } from "ethers/lib/utils";
import { useWeb3React } from "@web3-react/core";
import { ConditionalButton } from "@workhard/react-components";
...
export const Claim: React.FC<ClaimProps> = ({}) => {
  ...
  const [details, setDetails] = useState<
    (coingecko.CoingeckoTokenDetails | undefined)[]
  >([]);
  ...
  useEffect(() => {
    Promise.all(tokens.map((token) => coingecko.getPrice(token))).then(
      setPrices
    );
    Promise.all(tokens.map((token) => coingecko.getTokenDetails(token))).then(
      setDetails
    );
